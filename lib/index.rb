...
@headers = {
  ...
  "User-Agent": 'merged-to-staging action'
}
...
def run_check
  ...
  Dir.chdir(@GITHUB_WORKSPACE) do
    merged = `git branch --merged staging  | grep #{@branch_name}`
    if merged.length == 0
      conclusion = "failure"
    end
  end
  ...
  output = {
    "title": @check_name,
    "summary": conclusion == "success" ? "Branch #{@branch_name} merged in staging" : "Branch #{@branch_name} not merged in staging"
  }
